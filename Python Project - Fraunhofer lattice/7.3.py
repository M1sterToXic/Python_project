...
peak_lambda_values = {
    ...
    "Красный": 645e-9,     # Пик яркости в диапазоне 620–760 нм (где factor = 1.0)
    ...
    "Фиолетовый": 380e-9,  # Пик яркости в диапазоне 380–450 нм (где factor ближе к 1.0)
}
...
def plot_diffraction_pattern(ax, lambda_, d, L, light_type, zoom=1.0):
    ...
    # Определяем длину волны для масштабирования оси X
    if light_type == "Белый свет":
        reference_lambda_cm = 550e-9 * 100
    else:
        reference_lambda_cm = peak_lambda_values[light_type] * 100
    ...
    ax.fill_between(x_central, 0, y_central, color=central_color, alpha=0.8)
...
class DiffractionWindow(QMainWindow):
    def __init__(self):
        ...
        # Метка для масштаба
        self.zoom_label = QLabel("Масштаб: 1.0x")
